...
//Postcondition: Allocate space for a string object that represents a a string
//with the same value as the given c-string. The capacity of the string
//object will be set to be the one greater than is required to hold the string.
//As an example, the string "the" would set capacity at 4 instead of 3. A
//copy of the address of the opaqye object will be returned on success and NULL on failure.
MY_STRING my_string_init_c_string(const char* c_string);

//Precondition: hMy_string is the handle of a valid My_string object.
//Postcondition: Returns a copy of the integer value of object's capacity.
...
// we have read at least one non-whitespace character or if we reach the end of file.
// Function will return SUCCESS if a non-empty string is read successfull.
...
//  cannot resize to accommodate the new character then the operation fails
//  and FAILURE is returned. The resize operation will attempt to amortize
//  the cost of a resize by making the string capacity somewhat larger
...
// is unable to accommodate the characters in the hAppend string perhaps
...
//	MY_STRING objects.
//Postconditon: On success, hLeft will contain the handle
...
// attempt to resize to hild the data in hRight. If the object on the left
// already has enough room to store all the data on the right, then it
...
//any data with the object we are copying from. On failure, the funtion will
//return NULL.
MY_STRING my_string_init_copy(MY_STRING hMy_string);

//Precondition: hLeft and HRight are handles to valid MY_STRING objects.
//Postcondition: Aftre the function hLeft will contain the size capacity and
...
// Returns faillure in the case of a resizing problem with the new_key string.
